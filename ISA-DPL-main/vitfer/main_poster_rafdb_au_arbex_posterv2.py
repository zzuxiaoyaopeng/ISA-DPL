...
from vitfer.models.model_poster_vit_au_arbex_posterv2_bk4 import Poster
...
if __name__ == '__main__':
    ...
    # Logger
    logger = Logger(os.path.join("trained_weights", 'log.txt'), title='RAF-DB')
    ...
    # datasets(27:8 23:7 21:7 18:6 36:12)
    # (total:12271  0:1290 1:281 2:717 3:4772 4:1982 5:705 6:2524)
    train_img_path = '../datasets/train_images/'
    train_label_path = '../datasets/train_labels.txt'
    val_img_path = '../datasets/test_images/'
    val_label_path = '../datasets/test_labels.txt'
    data_train = Dataset_RAF(train_img_path, train_label_path, transform=transform_train, phase='train')
    data_val = Dataset_RAF(val_img_path, val_label_path, transform=transform_val, phase='val')
    print(len(data_train))
    print(len(data_val))
    data_loader_train = torch.utils.data.DataLoader(data_train, batch_size=36, shuffle=True, num_workers=2)
    data_loader_val = torch.utils.data.DataLoader(data_val, batch_size=64, shuffle=False, num_workers=2)
    ...
    data_loader_au_train = torch.utils.data.DataLoader(data_au_train, batch_size=12, shuffle=True, num_workers=2)
    ...
    # optimizer
    base_optimizer = Adan
    optimizer = SAM(model.parameters(), base_optimizer, rho=0.05, adaptive=False, lr=lr)
    ...
    for i in range(start_epoch, total_epochs):
        ...
        for step, data in enumerate(train_bar):
            # 训练阶段不建议打开autograd.detect_anomaly,会使训练速度大大减慢
