...
# RAFDB最佳建立2个LANet作为本地分支
# AffectNet最佳建立4个LANet作为本地分支,且drop为0.1(没用) -> 0.05
class LANet(nn.Module):
    def __init__(self, in_channel=512, ratio=32):
        super(LANet, self).__init__()
        self.con1x1_1 = nn.Conv2d(in_channel, in_channel // ratio, kernel_size=(1, 1))
        self.relu = nn.ReLU()
        self.con1x1_2 = nn.Conv2d(in_channel // ratio, 1, kernel_size=(1, 1))
...
class Local_CNNs(nn.Module):
    def __init__(self, channel=512, ratio=32):
        ...
        # self.LANet3 = LANet(channel, ratio)
        # self.LANet4 = LANet(channel, ratio)
    ...
    def forward(self, x):
        ...
        # M3 = self.LANet3(x.clone())  # [bs,1,7,7]
        # M4 = self.LANet4(x.clone())  # [bs,1,7,7]

        # drop处理(最好使用MAD)
        y1 = self.drop1(M1)  # [bs,1,7,7]
        y2 = self.drop2(M2)  # [bs,1,7,7]
        # y3 = self.drop3(M3)  # [bs,1,7,7]
        # y4 = self.drop4(M4)  # [bs,1,7,7]

        # max{y1,y2}
        Mout = torch.max(y1, y2)  # [bs,1,7,7]
        # y = torch.cat([y1, y2, y3, y4], dim=1)  # [bs,4,7,7]
        # Mout, _ = torch.max(y, dim=1, keepdim=True)  # [bs,1,7,7]

        # channel-wise multiplication
        Xout = torch.matmul(Mout, short)  # [bs,512,7,7]

        # # domain and pose invariance transform
        Xout = Xout + self.dp_conv1(Xout)
        Xout = Xout + self.dp_conv2(Xout)
        Xdp = self.drop_dp(Xout)

        # fuse short
        out = short + Xdp  # [bs,512,7,7]

        # [bs,512,7,7]->[bs,49,512]
