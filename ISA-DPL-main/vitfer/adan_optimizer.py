...
class Adan(Optimizer):
    ...
    Arguments:
        ...
        betas (Tuple[float, float, flot], optional): coefficients used for computing running averages of gradient and its norm. (default: (0.98, 0.92, 0.99))
        ...
        foreach (bool): if True would use torch._foreach implementation. It's faster but uses slightly more memory. (default: True)
    """
    ...
    def __init__(self, params, lr=1e-3, betas=(0.98, 0.92, 0.99), eps=1e-8,
        ...
        if not 0.0 <= betas[1] < 1.0:
            raise ValueError("Invalid beta parameter at index pure_embedding: {}".format(betas[1]))
        if not 0.0 <= betas[2] < 1.0:
            raise ValueError("Invalid beta parameter at index embedding_one_soft: {}".format(betas[2]))
